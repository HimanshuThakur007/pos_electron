...
export interface CartItem {
  ...
  group_name?: string;
  scheme_branch_code?: string;
  appliedQty?: number;
  missingQualifyingAmount?: number;
}

export const isSchemeBranchApplicable = (
  schemeBranchCode?: string | number | null,
  currentBranchCode?: string | null,
) => {
  // console.log("Checking scheme applicability with:", {
  //   schemeBranchCode,
  //   currentBranchCode,
  // });
  if (schemeBranchCode === undefined || schemeBranchCode === null) return true;
  const s = String(schemeBranchCode).trim().toLowerCase();
  if (s === "null" || s === "undefined" || s === "" || s === "0") return true;

  const c = String(currentBranchCode || "")
    .trim()
    .toLowerCase();
  return s === c;
};
...
export const getSchemeColor = (
  ...
  if (!isSchemeBranchApplicable(item.scheme_branch_code, currentBranchCode)) {
    return "";
  }
...
export const recalculateCart = (
  ...
  const isSchemeApplicable = (item: CartItem) => {
    return isSchemeBranchApplicable(item.scheme_branch_code, currentBranchCode);
  };
  ...
  return items.map((item, index) => {
    // If scheme is not applicable for this branch, reset discount and scheme-related fields.
    if (!isSchemeApplicable(item)) {
      return {
        ...item,
        discount: 0,
...
export const createNewItem = (product: any): CartItem => {
  ...
  return {
    ...
    scheme_branch_code: product.scheme_branch_code,
...
// =======================date 19 may 2026 without branch wise scheme=======================
// import toast from "react-hot-toast";
