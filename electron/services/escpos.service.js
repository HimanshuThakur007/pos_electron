...
// // ================= CONFIG =================
// const LINE_WIDTH = 42; // change to 32 (58mm) or 48 (80mm)
...
//     const target =
//       printers.find((p) =>
//         p.name.toLowerCase().match(/epson|pos|receipt/)
//       ) ||
...
//     printer.println(center(`CIN: ${data.storeDetails.cin || "U18204DL2008PTC175779"}`));
//     printer.println(center("info@market99.com, www.market99.com"));
//     printer.println(center("Regd Off: D-153, Okhla Ph-1, N.D-20 Ph.No. 011-47366100"));
...
//     printer.println(leftRight(`Date: ${data.billDetails.date}`, `Time: ${time}`));

//     printer.println(line());

//     // ===== TABLE HEADER =====
//     printer.bold(true);
//     printer.println(col("Description", 30) + col("Hsn", 12, "RIGHT"));
//     printer.println(
//       col("Barcode", 10) +
//       col("Qty", 4, "CENTER") +
//       col("Price", 8, "RIGHT") +
...
//         const descLines = wrapText(item.printDesc || item.itemName, 30);

//         descLines.forEach((lineText, i) => {
//           printer.println(
//             col(lineText, 30) +
//             col(i === 0 ? item.hsn || "610990" : "", 12, "RIGHT")
//           );
//         });

//         printer.println(
//           col(item.itemCode, 10) +
//           col(item.qty, 4, "CENTER") +
//           col(item.price.toFixed(2), 8, "RIGHT") +
//           col(item.discount.toFixed(2), 8, "RIGHT") +
...
//     printer.bold(true);
//     printer.println(
//       col(`Items:${data.cart.length}`, 14) +
//       col(`Qty:${data.totals.totalQty}`, 14) +
//       col(`Disc:${data.totals.totalDiscount.toFixed(2)}`, 14)
//     );
//     printer.bold(false);

//     printer.println(line());

//     printer.println(leftRight("Gross Sale Value", data.totals.grossAmount.toFixed(2)));
...
//     printer.bold(true);
//     printer.println(leftRight("Net Payable", data.totals.grandTotal.toFixed(2)));
//     printer.bold(false);

//     printer.println(leftRight("Received Amount", (data.totals.amountReceived || 0).toFixed(2)));
//     printer.println(leftRight("Balance Paid", (data.totals.balance || 0).toFixed(2)));
...
//     printer.println(
//       col(data.billDetails.paymentMode || "CASH", 10) +
//       col(data.billDetails.transactionRef || "xxxxxxxxxxx", 20, "CENTER") +
//       col((data.totals.amountReceived || data.totals.grandTotal).toFixed(2), 12, "RIGHT")
//     );
...
//       printer.println(
//         col(rate + "%", 6) +
//         col(taxable.toFixed(2), 10, "RIGHT") +
//         col((tax / 2).toFixed(2), 8, "RIGHT") +
//         col((tax / 2).toFixed(2), 8, "RIGHT") +
    ...
    for (const rate of sortedRates) {
      ...
      grouped[rate].forEach((item) => {
        ...
        descLines.forEach((lineText, i) => {
          printer.println(
            col(lineText, 34) +
              col(i === 0 ? item.hsn || "610990" : "", 14, "RIGHT"),
          );
        });
