...
export const getStockByLogicUserCodeSqlite = (logicUserCode) => {
  try {
    ...
    const itemsTableExists = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='items'`).get();
    const offersTableExists = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='m99_reg_offer'`).get();
    ...
    if (itemsTableExists) {
      ...
      if (offersTableExists) {
        selectFields += `, t4.schm_type, t4.schm_camp_grp, t4.itm_code`;
    ...
    return Object.values(groupedMap).map(item => ({
      ...
      schm_camp_grp: item.schemes && item.schemes.size > 0 ? Array.from(item.schemes).join(" ") : null,
...
export const createTempStockTableSqlite = (sampleItem) => {
  try {
    ...
    const INT_FIELDS = [
      "Branch_Code", "Lot_Code", "Godown_Code", "Item_Cf_1", "Item_Cf_2", "Item_Cf_3", "Bin_Code",
    ];

    const REAL_FIELDS = [
      "Stock_Qty", "Carton_Stock", "Lot_Sale_Rate", "Lot_Basic_Rate", "Lot_MRP",
      "Lot_SPRate2", "Lot_SPRate3", "Lot_SPRate4", "Item_Sale_Rate", "Item_MRP", "Lot_SPRate1",
    ];
  ...
  try {
    ...
    const updateTransaction = db.transaction((items) => {
      for (const item of items) {
        const row = findStmt.get({ itemCode: item.itemCode, price: item.price });
        if (row) {
...
export const createStockIndexesSqlite = () => {
  try {
    ...
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_logic_user_code_temp ON ${TEMP_TABLE_NAME} (LogicUserCode COLLATE NOCASE)`).run();
    
    // Index for AddlItemCode (Barcode) - Case Insensitive
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_addl_item_code_temp ON ${TEMP_TABLE_NAME} (AddlItemCode COLLATE NOCASE)`).run();
  } catch (err) {
...
export const insertSyncLogSqlite = (status, message) => {
  try {
    ...
    `).run();
    db.prepare(`INSERT INTO ${LOG_TABLE_NAME} (status, message) VALUES (?, ?)`).run(status, message);
  } catch (err) {
...
export const getLastSyncLogSqlite = (status = null) => {
  try {
    const tableExists = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name=?`).get(LOG_TABLE_NAME);
    if (!tableExists) return null;

    if (status) {
      return db.prepare(`SELECT * FROM ${LOG_TABLE_NAME} WHERE status = ? ORDER BY id DESC LIMIT 1`).get(status);
    }

    return db.prepare(`SELECT * FROM ${LOG_TABLE_NAME} ORDER BY id DESC LIMIT 1`).get();
  } catch (err) {
    return null;
  }
};
...
export const clearSyncLogsSqlite = () => {
  try {
    db.prepare(`DELETE FROM ${LOG_TABLE_NAME}`).run();
    db.prepare(`DELETE FROM sqlite_sequence WHERE name='${LOG_TABLE_NAME}'`).run();
  } catch (err) {
